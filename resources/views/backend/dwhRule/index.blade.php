...
@section('content_header')
    <h1>{{ trans('dwhRule.backend.title') }}</h1>
@stop

@section('content')
    @include('backend.errors.success')
    @include('backend.errors.error')
    @include('backend.dwhRule.partials.search_box')
    ...
    <script type="text/javascript">
        ...
        jQuery(function ($) {
            ...
            $('#grid').w2grid({
                ...
                multiSearch: true,
                searches: [
                    {field: 'code', caption: '{{ trans('dwhRule.backend.table.table') }}', type: 'text'},
                    {field: 'name', caption: '{{ trans('dwhRule.backend.table.to_dwh_days') }}', type: 'text'},
                    {field: 'desc', caption: '{{ trans('dwhRule.backend.table.delete_from_dwh_days') }}', type: 'text'},
                ],
                columns: [
                    {
                        field: 'recid',
                        ...
                        field: 'table',
                        caption: '{{ trans('dwhRule.backend.table.table') }}',
                        size: '35%',
                        sortable: true,
                        attr: 'align=left'
                    }, {
                        field: 'job_log_type',
                        caption: '{{ trans('dwhRule.backend.table.job_log_type') }}',
                        size: '35%',
                        sortable: true,
                        attr: 'align=left'
                    }, {
                        field: 'description',
                        caption: '{{ trans('dwhRule.backend.table.description') }}',
                        sortable: true,
                        attr: 'align=left'
                    }, {
                        field: 'to_dwh_days',
                        caption: '{{ trans('dwhRule.backend.table.to_dwh_days') }}',
                        size: '35%',
                        sortable: true,
                        attr: 'align=left'
                    },
                    {
                        field: 'delete_from_dwh_days',
                        caption: '{{ trans('dwhRule.backend.table.delete_from_dwh_days') }}',
                        size: '30%',
                        sortable: true,
                        attr: 'align=left'
                    }
                ...
                records: [
                    @include('backend.dwhRule.partials.table')
                ]
            });
        });
    </script>
