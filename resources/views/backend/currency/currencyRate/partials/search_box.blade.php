    ...
    <div class="box-body">
        @if (isset($data))
            <form action="{{ url()->current() }}?{{ implode(',', $data) }}" method="GET">
                @endif
                @include('backend.currency.currencyRate.partials.fields_search_box')
                <div class="form-group">
                        <div for="number" class="col-sm-5 control-label"></div>
                        {!! Form::submit('Поиск', ['class' => 'btn btn-primary']) !!}
                        <a type="submit" class="btn btn-danger" style="margin:0px 8px" href="{!! route('admin.currencies.rates.index') !!}">
                            Сбросить
                        </a>
