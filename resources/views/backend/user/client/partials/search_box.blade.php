    ...
    <div class="box-body">
        @if (isset($data))
            <form action="{{ url()->current() }}?{{ implode(',', $data) }}" method="GET">
                @endif
                @include('backend.user.client.partials.fields_search_box')
                <div class="form-group">
                        <div for="number" class="col-sm-5 control-label"></div>
                        {!! Form::submit('Поиск', ['class' => 'btn btn-primary','name'=>'search']) !!}<a type="submit" class="btn btn-danger" href="{!! route('admin.clients.index') !!}" style="margin: 0 8px;">Сбросить</a>
