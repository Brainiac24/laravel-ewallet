<div class="box box-default form-horizontal @if($filterFields==[])collapsed-box @endif">
    ...
    <div class="box-body">
        @if (isset($filterFields))
            <form action="{{ url()->current() }}?{{ implode(',', $filterFields) }}" method="GET">
                @endif
                @include('backend.order.orderCardType.partials.fields_search_box')
                <div class="form-group">
                    <div for="number" class="col-sm-5 control-label"></div>
                    {!! Form::submit('Поиск', ['class' => 'btn btn-primary','name'=>'search']) !!}<a type="submit"
                                                                                                     class="btn btn-danger"
                                                                                                     href="{!! route('admin.order.orderCardType.index') !!}"
