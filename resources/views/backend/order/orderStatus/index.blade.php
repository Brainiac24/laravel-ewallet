...
@section('content_header')
    <h1>{{ trans('orderStatus.backend.title') }}</h1>
@stop
    ...
    <script type="text/javascript">
        ...
        jQuery(function ($) {
            ...
            $('#grid').w2grid({
                ...
                multiSearch: true,
                searches: [
                    {field: 'code', caption: '{{ trans('orderStatus.backend.table.code') }}', type: 'text'},
                    {field: 'name', caption: '{{ trans('orderStatus.backend.table.name') }}', type: 'text'},
                ],
                columns: [
                    {
                        field: 'recid',
                    ...
                    {
                        field: 'code',
                        caption: '{{ trans('orderStatus.backend.table.code') }}',
                        size: '20%',
                        sortable: true,
                        attr: 'align=left'
                    },
                    {
                        field: 'name',
                        caption: '{{ trans('orderStatus.backend.table.name') }}',
                        size: '65%',
                        sortable: true,
                        attr: 'align=left'
                    },
                ...
                records: [
                    @include('backend.order.orderStatus.partials.table')
                ]
            });
        });
    </script>
