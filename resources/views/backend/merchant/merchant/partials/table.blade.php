...
@foreach($merchants as $item)
{
    ...
    contract_date_at: '{{ empty($item->contract_date_at)?null:\Carbon\Carbon::parse($item->contract_date_at)->format("d.m.Y H:i:s")}}',
    login:'{{$item->login}}',
    is_qr_integrated:'{{trans('InterfaceTranses.verified.'.$item->is_qr_integrated??0 )}}',
    document:'{{trans('InterfaceTranses.verified.'.$item->document??0 )}}',
    referral:'{{$item->params_json['referral']??''}}',
},
@endforeach